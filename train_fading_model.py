...
class CommsNet(nn.Module):
    '''
    Neural network for constellation prediction with fading channels
    '''

    def __init__(self):
        super(CommsNet, self).__init__()
        self.n_channels = 8
        self.n_constellation_points = 64
        self.conv1 = nn.Sequential(
            nn.Conv2d(in_channels=1,
                      out_channels=self.n_channels,
                      kernel_size=(4, 1), stride=(2, 1), padding=(2, 0)),
            nn.MaxPool1d
        )
        self.conv2 = nn.Sequential(
            nn.Conv2d(in_channels=self.n_channels,
                      out_channels=self.n_channels*self.n_constellation_points,
                      kernel_size=(2, 1), groups=self.n_channels)
        )

        self.fc = nn.Sequential(
            nn.Linear(self.n_channels * self.n_constellation_points, 48),
            nn.ReLU(),
